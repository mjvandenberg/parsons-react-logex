...
import { ParsonsUiItem, ParsonsProblemProperties, Settings, ParsonsItem } from '../types';
  ...
  ...props
    ...
    const handleOnClickShowStep = () => {
      ...
      const distractorsFromListRight = listRight.reduce<ParsonsUiItem[]>(
        (previousValue, currentItem, currentIndex, arr) => {
          const solutionItem = exerciseSolution.find(i => i.text === currentItem.text && i.type === "block");
      ...
      if (distractorsFromListRight.length > 0) {
        ...
        setListRight(listRight.reduce<ParsonsUiItem[]>(
          (previousValue, currentItem, currentIndex, arr) => {
    ...
    return (
      <>
        ...
        {showHint && hintItem && <div className="basis-0 bg-[#fcfcfc] rounded border-[#b8b8b8] border-1 border p-2 m-3">
          <strong>Hint:</strong> Try to drop the block with formula <span className="katex">{hintItem.text}</span> somewhere in the solution area, it is part of the solution! {!settings.instantFeedback && "You can click on 'Check me' to validate your solution."}
        </div>}
        {showHint && !hintItem && <div className="basis-0 bg-[#fcfcfc] rounded border-[#b8b8b8] border-1 border p-2 m-3">
          <strong>Sorry, no hint available.</strong>
        </div>}
